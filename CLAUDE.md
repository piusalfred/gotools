...
- **Offline mode:** `--offline` / `GOTOOLS_OFFLINE=1` (`_parse_offline` in core.sh, `_OFFLINE` global) makes CI hermetic. `sync` takes ONLY the fingerprint fast path and exits `E_OFFLINE` (6) otherwise; `install`/`upgrade` refuse outright. This is the first live use of the reserved `E_OFFLINE` exit code — keep the code's meaning: "network needed but --offline set".
...
- **Locking:** `_acquire_lock` (core.sh) honors `GOTOOLS_LOCK_TIMEOUT` (default 10s), `GOTOOLS_LOCK_STALE_TIMEOUT` (default 300s — `_lock_detect_stale` removes crashed-process locks), and `GOTOOLS_NO_LOCK=1`. Offline sync runs BEFORE lock acquisition (read-only). All `go` invocations in install/sync/upgrade/migrate/remove/restore go through `_go` → `_go_with_timeout` (`GOTOOLS_OPERATION_TIMEOUT`, default 120s, 0 disables; timed-out ops exit `E_NETWORK`). `cmd_exec`'s `go tool` calls are deliberately NOT wrapped — they run the user's tool. When adding new `go` calls, route them through `_go`, never call `go` directly.

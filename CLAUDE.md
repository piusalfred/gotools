...
- **Sync fast path:** `cmd_sync` skips reconciliation when `_sync_fingerprint` (SHA-256 of the tool list plus strategy, resolved Go version, and module prefix) matches `$GOTOOLS_DIR/.gotools.fingerprint` AND every modfile still carries the manifest version. The reinstall loop is version-aware: missing or drifted modfiles are reinstalled via `cmd_install --force`. The fingerprint is written only after a successful sync and is committed to the repo (deterministic, so fresh clones hit the fast path). `rename` also refreshes it (via `_sync_write_fingerprint`) after moving disk state, so a sync right after rename fast-paths — other mutating commands still leave it stale (ISS-028).
- **go.mod adoption / restoration:** `init` adopts tools already declared in the root `go.mod` (scan → install → strip → tidy; `--no-migrate` opts out, `--dry-run` previews; the root `go.mod` is untouched until all installs succeed, and re-running `init` merges rather than wipes the manifest). `purge --restore` is the reverse: it re-adds every managed tool to the root `go.mod` via `go get -tool pkg@version` BEFORE wiping gotools state, so a failed restore loses nothing. Helpers live in `lib/modparser.sh` (`strip_tools_from_mod`, `_gomod_tool_scan`).
- **Rename:** `_rename_tool` (cmd_rename.sh) is the shared core — it assumes the caller holds the lock and loaded config (`cmd_install`'s prompt path reuses it; never call `cmd_rename` itself from inside another command or it re-acquires the lock). Split moves `tools/<old>.mod`/`.sum` and rewrites the module line via `go mod edit -module` (rolled back on failure); module moves the directory; unified REFUSES (names come from the tool directives — a manifest-only rename desyncs list/info/exec). After moving disk state it refreshes the sync fingerprint, so a follow-up `sync` (even `--offline`) fast-paths. `cmd_install` detects the same package under a different name (`_manifest_tool_find_by_package`): on a TTY it asks install-another / upgrade-existing / rename / skip; on non-interactive stdin it informs and installs; same name+package still refuses without `--force`.
- **Offline mode:** `--offline` / `GOTOOLS_OFFLINE=1` (`_parse_offline` in core.sh, `_OFFLINE` global) makes CI hermetic. `sync` takes ONLY the fingerprint fast path and exits `E_OFFLINE` (6) otherwise; `install`/`upgrade` refuse outright. This is the first live use of the reserved `E_OFFLINE` exit code — keep the code's meaning: "network needed but --offline set". `doctor --offline` is read-only diagnostics: it skips the proxy check and runs the tool/module probes with `GOPROXY=off` (fail fast, no network) — it does NOT exit `E_OFFLINE`.
- **doctor (read-only diagnostics):** `cmd_doctor` runs seven checks (Go, proxy, config, strategy-vs-disk, tools, lock, integrity, disk) into a `_DOCTOR_RESULTS` pipe-delimited store (`name|status|detail`, statuses pass|warn|fail|skip), then renders text or JSON. It **always exits 0** — diagnostics never fail the shell (CI gates on `--format=json` → `healthy`/`issues`); invalid flags still exit `E_USAGE`. It never acquires the lock, never writes manifest/fingerprint/trace, and probes set `GOTOOLCHAIN=local` so doctor never auto-downloads a toolchain. Single-result checks run through `_doctor_run_check <name> <fn>` (subshell capture; fn echoes detail to stdout and returns 0/1/2/other → pass/warn/fail/skip); the multi-result tools check runs in the main shell and appends directly (subshell mutations would be lost). `load_config` EXITS on a newer-schema manifest, so `cmd_doctor` pre-validates via `_manifest_validate` and bypasses `load_config` (replicating env>default precedence) to still render a report. `go list -m` accepts no `-timeout` flag — the proxy reachability probe uses a 5s bash watchdog around `go list -m <first-tool>@latest` (`@latest` always hits the network; pinned `@version` can be cache-served).
- **Manifest `settings` block:** runtime knobs (`offline`, `jobs`, `operation_timeout`, `lock_timeout`, `lock_stale_timeout`, `no_lock`, `trace`, `verbose`) live in a `settings` block with **flag > env > manifest > default** precedence. `_settings_resolve` (manifest.sh) resolves a key; `_ORIG_ENV_*` captures in config.sh mirror the layout keys so an explicit `GOTOOLS_X=0` can override a manifest `true`. `_manifest_flush` always emits ALL eight keys (defaults filled) — visibility by default; `init` gets them for free since it flushes. `_manifest_config_set`/`_settings_set` keep the in-memory `_SETTINGS` store (pipe-delimited `key|value` lines) in sync and update the resolved globals. The block is additive — no `_MANIFEST_SCHEMA_VERSION` bump. `cmd_config` accepts `KEY VALUE` and `KEY=VALUE` and validates values (exit `E_USAGE` on bad input). Trace streams: `_TRACE` = `true`/`1` → stderr + file; `stdout` → stdout + file; file append is always the durable record.
- **Machine-readable output:** `list`, `info`, `doctor`, `version` take `--format=json|text` (+ `--json`/`--text` aliases) parsed by `_parse_output_format` (core.sh, sets `_OUTPUT_FORMAT`). JSON is write-only; every document carries `"schema_version":1` — a SEPARATE version from `_MANIFEST_SCHEMA_VERSION` (bump it when a JSON output shape changes). All JSON values go through `_json_escape`. Unknown `--format=<x>` and stray positional args exit `E_USAGE`.
- **bash 3.2 gotcha (macOS /bin/bash):** array-expanded env assignments before a FUNCTION call fail with `command not found` — `arr=(FOO=bar); "${arr[@]}" myfunc` does NOT work on bash 3.2 (a literal `FOO=bar myfunc` does). Never use that pattern; `export` the vars inside a subshell instead: `(export GOTOOLCHAIN=local; ... _go ...)`. See `tool_runnable` and `_doctor_check_integrity`.
- **Parallel sync:** `sync --jobs N` / `GOTOOLS_JOBS=N` reinstalls missing/drifted tools concurrently — OPT-IN, the default is serial (jobs=1, the user's explicit preference). Only split/module parallelize (independent modfiles); unified stays serial with a warning. `_parallel_for` (core.sh) is the bash-3.2-portable semaphore (no `wait -n`; subshell-wrapped background jobs work around the 3.2 errexit/& bug; reap only when the cap is EXCEEDED or the semaphore degenerates to serial). Parallel jobs run `cmd_install` with `_INSTALL_NO_FLUSH=1` so they never race on the manifest — the parent merges and flushes once after ALL jobs succeed (all-or-nothing on failure).
...
- **Locking:** `_acquire_lock` / `_release_lock` use a directory-based mutex (`mkdir` is atomic) to prevent concurrent operations. The lock dir carries a `pid` file recording the holder — `_lock_detect_stale` (core.sh) treats a lock with a LIVE pid (`ps -p` probe, not `kill -0`, which reports EPERM as dead for other users' processes) as never stale, a dead pid as always stale, and pid-less legacy locks via an age check (`GOTOOLS_LOCK_STALE_TIMEOUT`, default 300s). `_release_lock` must remove the pid file before `rmdir` (rmdir only removes empty dirs). `GOTOOLS_LOCK_TIMEOUT` (default 10s, 0.5s poll) bounds the wait; `GOTOOLS_NO_LOCK=1` skips acquisition (CI serial-workspace escape hatch). `sync --offline` never acquires the lock — it cannot write, so concurrent offline CI jobs never contend.
- **Operation timeouts:** Go has NO built-in timeout for module operations (`go get -tool`, `go mod tidy`, `go mod edit` reject `-timeout`; `GOFLAGS=-timeout` is silently ignored — verified empirically), and macOS has no `timeout(1)`. `_go_timeout` (core.sh) is the single entry point for network-bound go calls: `GOTOOLS_OPERATION_TIMEOUT` (default 120s, `0` disables) → GNU `timeout(1)` when present → portable `_run_with_timeout` watchdog (backgrounds the command with stdout/stderr flowing through unchanged, polls `kill -0` every 0.5s, escalates TERM→KILL; safe inside `$()` — killing the child closes the pipe). **rc 124 is the canonical "timed out" code** across both mechanisms; call sites with their own error handling check it and exit `E_NETWORK` with a clear message, bare sites use `_timeout_fatal <rc> <what>`. Timeouts in `cmd_install` must also run the `_created_mod` cleanup. `exec` is deliberately NOT bounded (tools run for minutes). Doctor's proxy check uses `_run_with_timeout 5` directly — extend there, don't write a new watchdog.
